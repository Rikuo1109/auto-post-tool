...
class GetMatrixPostManagementService:
    ...
    def __call__(self):
        ...
        if self.filters.min_time:
            post_managements = post_managements.filter(created_at__gte=self.filters.min_time)
        if self.filters.max_time:
            post_managements = post_managements.filter(created_at__lte=self.filters.max_time)
        # TODO: update sorting feature later
        # post_managements = self.filter_sorting(post_managements)
        post_managements = self.filter_paging(post_managements)
